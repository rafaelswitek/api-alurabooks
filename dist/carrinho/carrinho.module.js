"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CarrinhoModule = void 0;
const common_1 = require("@nestjs/common");
const livros_module_1 = require("../livros/livros.module");
const carrinho_resolver_1 = require("./resolvers/carrinho.resolver");
const carrinho_service_1 = require("./services/carrinho.service");
let CarrinhoModule = class CarrinhoModule {
};
CarrinhoModule = __decorate([
    (0, common_1.Module)({
        providers: [carrinho_service_1.CarrinhoService, carrinho_resolver_1.CarrinhoResolver],
        imports: [livros_module_1.LivrosModule],
    })
], CarrinhoModule);
exports.CarrinhoModule = CarrinhoModule;
//# sourceMappingURL=carrinho.module.js.map